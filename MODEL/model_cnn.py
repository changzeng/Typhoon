    ...
    def __init__(
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-filter{0}".format(filter_size)):
                # Convolution Layer
                filter_shape = [filter_size, embedding_size, 1, num_filters]
                W_front = tf.Variable(tf.truncated_normal(shape=filter_shape, stddev=0.1, dtype=tf.float32), name="W_front")
                b_front = tf.Variable(tf.constant(0.1, shape=[num_filters], dtype=tf.float32), name="b_front")
                W_behind = tf.Variable(tf.truncated_normal(shape=filter_shape, stddev=0.1, dtype=tf.float32), name="W_behind")
                b_behind = tf.Variable(tf.constant(0.1, shape=[num_filters], dtype=tf.float32), name="b_behind")
                conv_front = tf.nn.conv2d(
                    self.embedded_sentence_expanded_front,
                    W_front,
                    strides=[1, 1, 1, 1],
                ...
                # Batch Normalization Layer
                conv_bn_front = tf.layers.batch_normalization(tf.nn.bias_add(conv_front, b_front), training=self.is_training)
                conv_bn_behind = tf.layers.batch_normalization(tf.nn.bias_add(conv_behind, b_behind), training=self.is_training)
